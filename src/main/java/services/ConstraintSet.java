 ...
 *
 * <p>The set always contains the full list of supported constraints (see
 * {@link #defaults()}). User edits update the existing entries in-place rather
 * than removing/recreating them.</p>
 */
public class ConstraintSet {
    ...
    public static ConstraintSet defaults() {
        ...
        set.put(new Constraint(ConstraintIds.RESPECT_PROF_UNAVAILABILITY,
                ...
                "true", ConstraintPriority.HARD));
        set.put(new Constraint(ConstraintIds.JURY_SIZE,
                "Taille du jury (encadrant + N rapporteurs, total)",
                "3", ConstraintPriority.HARD));
        return set;
    }
    ...
    /**
     * Total jury size per soutenance: encadrant (president) + N rapporteurs.
     * Minimum 2 (encadrant + 1 rapporteur). No upper cap — the user decides.
     * Default is 3.
     */
    public int getJurySize() {
        Constraint c = get(ConstraintIds.JURY_SIZE);
        return c == null ? 3 : Math.max(2, c.asInt(3));
    }
