...
public class RecommendationServiceImpl implements RecommendationService {
    ...
    public List<Recommendation> generateRecommendations(int totalProjects, int numberOfRooms, int slotsPerDay,
        ...
        // ── Capacity & Prof workload check ──────────────────────────────────
        int dailyRoomCapacity = Math.max(0, numberOfRooms) * Math.max(0, slotsPerDay);
        int totalCapacity = dailyRoomCapacity * Math.max(0, numberOfDays);
        int recommendedDaysRoom = dailyRoomCapacity > 0 ? (int) Math.ceil(totalProjects / (double) dailyRoomCapacity) : 0;
        ...
        // Each project needs 3 profs, so total prof slots = totalProjects * 3
        int recommendedDaysProf = profCapacityPerDay > 0 ? (int) Math.ceil((totalProjects * 3.0) / profCapacityPerDay) : 0;

        int recommendedDays = Math.max(recommendedDaysRoom, recommendedDaysProf);
        ...
        if (totalCapacity > 0) {
            if (totalCapacity < totalProjects) {
                ...
                out.add(Recommendation.error(
                        "Capacite en salles insuffisante",
                ...
                out.add(Recommendation.info(
                        "Capacite globale suffisante",
                        "Capacite totale (salles): " + totalCapacity + " creneaux. " + totalProjects + " projets a planifier."));
            }
        }
        ...
        if (recommendedDays > 0 && numberOfDays > 0 && numberOfDays < recommendedDays) {
            out.add(Recommendation.warning(
                    ...
                    "Vous avez " + totalProjects + " projets. Pour respecter les capacites (salles et profs), recommandation: au moins " + recommendedDays
        ...
        if (numberOfProfs > 0 && totalProjects > 0) {
            ...
            int profCapacityPerDayCheck = numberOfProfs * maxPerProfPerDay;
            int neededProfSlotsPerDay = (int) Math.ceil(totalProjects * 3.0 / Math.max(1, numberOfDays));
            if (profCapacityPerDayCheck > 0 && neededProfSlotsPerDay > profCapacityPerDayCheck) {
                out.add(Recommendation.warning(
                        "Limite par prof/jour serree",
                        "Limite actuelle: " + maxPerProfPerDay + " soutenances/prof/jour x " + numberOfProfs
                                + " profs = " + profCapacityPerDayCheck + " participations/jour. Besoin estime: "
                                + neededProfSlotsPerDay + ".",
                        "Augmentez 'Max soutenances/prof/jour' ou ajoutez des jours. (Recommande: " + recommendedDays + " jours minimum)"));
            }
        }
